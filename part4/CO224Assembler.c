	...
	while (fgets(line, LINE_SIZE, fi) != NULL) // Read a line from the input .s file
	{
		...
		*************************************************************************/
			...
			strcasecmp(in_token, "swd") == 0 || strcasecmp(in_token, "swi") == 0 || strcasecmp(in_token, "j") == 0)
		{
			...
			strcat(pline, delim); // Write the operation to the pre-processed line, with a delimiter
			...
			if (store) // If this is a store instruction which has no RD field but, a RT field
			{
				...
				strcat(pline, in_token);
				strcat(pline, delim); // Write RT to the pre-processed line, with a delimiter
			}
			else // Otherwise
			{
				in_token = strtok(NULL, delim); // Read the second token (RD)
				strcat(pline, in_token);
				strcat(pline, delim); // Write RD to the pre-processed line, with a delimiter
			...
			while (1)
			{
				...
				{
					strcat(pline, delim);
					strcat(pline, in_token); // Write any remaining tokens to the pre-processed line
				}
				else
					break;
			}
		...
		*******************************************************************/
		...
		while (in_token != NULL)
		{
			...
			else if (strcasecmp(in_token, "j") == 0)
				strcpy(out_token, op_j);
			else if (strcasecmp(in_token, "beq") == 0)
				strcpy(out_token, op_beq);
